/*******************************************************************************
 * This software is released under the licence CeCILL
 * 
 * see Licence_CeCILL-C_fr.html see Licence_CeCILL-C_en.html
 * 
 * see <a href="http://www.cecill.info/">http://www.cecill.info/a>
 * 
 * @copyright IGN
 ******************************************************************************/
package fr.ign.cogit.cartagen.core.genericschema.railway;

import fr.ign.cogit.cartagen.core.genericschema.IGeneObjLin;

/*
 * ###### IGN / CartAGen ###### Title: RailwayRoute Description: Lignes de
 * chemin de fer Author: J. Renard Date: 16/09/2009
 */

public interface IRailwayRoute extends IGeneObjLin {
  public static final String FEAT_TYPE_NAME = "RailwayRoute"; //$NON-NLS-1$
}
